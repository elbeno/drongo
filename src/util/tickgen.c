...
/* Tick generator. Sends @comp every 6 seconds. For most uber servers,
   this will need changing to something like '@.gen_tick();\n@comp' -
   see #define cmmsg - since the standard is for uber to do an @flush
   after an @comp
*/
...
char *av[];
{
 ...
 if(fork()) {
  ...
  /* send flush message */
  if(vflg)
   (void)fprintf(stderr,"<<<flushing>>>\n");
  sendto_server(servfd,flmsg);

  /* send tick call message */
  if(vflg)
   (void)fprintf(stderr,"<<<sending tick message>>>\n");
  sendto_server(servfd,tickmsg);
